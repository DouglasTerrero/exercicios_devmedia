...
function embaralha(colecao){
  return colecao.sort(() => Math.random() - 0.5);
}

function selecionaAleatorio(colecao, quantidade){
  const questoesEmbaralhadas = embaralha(colecao);
  return questoesEmbaralhadas.slice(0, quantidade);
}

function exibe(colecao){
  return colecao.forEach((pergunta, index) => {
    console.log(`${index+1}: ${pergunta.pergunta}`)
    let resposta = leitor.question("Digite a resposta(ano): ");
...
let qtdQuestoes = leitor.question("\nSelecione o numero de questoes(maximo 30): ");

let questoesSelecionadas = selecionaAleatorio(questoes, qtdQuestoes);
exibe(questoesSelecionadas);

let porcentagem = certas/qtdQuestoes;



console.log("\nJogador(a): "+ nome);
console.log("Pontuacao final: "+ certas+ " de "+qtdQuestoes);
console.log("Porcentagem de acerto: "+ (porcentagem * 100)+"%.");
...
else if ((porcentagem) > 0.5 && (porcentagem) < 1){
